...
export default function TabelaEquipamentos() {
  const [equipamentos, setEquipamentos] = React.useState(DADOS_EQUIPAMENTOS_INICIAIS);
  ...
  {eq.status === "Em estoque" ? (
    ...
    <Button 
      variant="contained" 
      color="error" // Cor vermelha do tema Material-UI
      size="small"
    >
      Em Uso
    </Button>
  ) : (
    // Se não for nem 1 nem 2, ou para outros status (Ex: Em Manutenção, Descartado)
    <Button 
      variant="contained" 
      color="warning" // Cor amarela/laranja do tema Material-UI
      size="small"
    >
      Outro Status
    </Button>
